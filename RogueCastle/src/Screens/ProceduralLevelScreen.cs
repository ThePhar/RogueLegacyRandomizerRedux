...
public class ProceduralLevelScreen : Screen
{
    ...
    private float m_elapsedScreenShake;

    private          EnemyHUDObj m_enemyHUD;
    private          float       m_enemyHUDCounter;
    private readonly float       m_enemyHUDDuration = 2.0f;
    ...
    private readonly float          m_fakeElapsedTotalHour = 1f / 60f / (60f * 60f);
    ...
    private   bool           m_toggleMagentaBG;
    ...
    {
        get => m_miniMapDisplay.AddedRoomsList;
        set
        {
    ...
    public List<RoomObj> MapRoomsAdded => m_miniMapDisplay.AddedRoomsList;
    ...
    public RoomObj CurrentRoom => m_currentRoom;
    ...
    public TextManager TextManager => m_textManager;

    public ImpactEffectPool ImpactEffectPool { get; private set; }

    public ItemDropManager ItemDropManager => m_itemDropManager;

    public GameTypes.LevelType CurrentLevelType => m_currentRoom.LevelType;

    public int LeftBorder => m_leftMostBorder;

    public int RightBorder => m_rightMostBorder;

    public int TopBorder => m_topMostBorder;

    public int BottomBorder => m_bottomMostBorder;
    ...
    {
        var expandedRoomRect = new Rectangle((int) room.X - 100, (int) room.Y - 100, room.Width + 200, room.Height + 200); // An expanded bounds of the room.
        ...
        foreach (var obj in m_projectileManager.ActiveProjectileList)
        {
            m_physicsManager.AddObject(obj);
        }
        ...
        foreach (var room in RoomList)
        {
            foreach (var enemy in room.EnemyList)
            {
                ...
                if (room.Name == "Boss" && room.LinkedRoom != null)
                {
                    ...
                    var bossEnemyLevel = (int) (roomLevel / (LevelEV.ROOM_LEVEL_MOD + Game.PlayerStats.GetNumberOfEquippedRunes(EquipmentAbilityType.RoomLevelDown) * GameEV.RUNE_GRACE_ROOM_LEVEL_LOSS));
                    enemy.Level = bossEnemyLevel;
                }
                else
                {
                    var enemyLevel = (int) (roomLevel / (LevelEV.ROOM_LEVEL_MOD + Game.PlayerStats.GetNumberOfEquippedRunes(EquipmentAbilityType.RoomLevelDown) * GameEV.RUNE_GRACE_ROOM_LEVEL_LOSS));
                    if (enemyLevel < 1)
                    {
                        enemyLevel = 1;
                    }
    ...
    public void OpenChallengeBossDoor(RoomObj linkerRoom, GameTypes.LevelType levelType)
    {
        ...
        {
            case GameTypes.LevelType.CASTLE:
                if (Game.PlayerStats.EyeballBossBeaten && Game.PlayerStats.ChallengeEyeballBeaten == false && Game.PlayerStats.ChallengeEyeballUnlocked)
                {
                ...
                break;
            case GameTypes.LevelType.DUNGEON:
                if (Game.PlayerStats.BlobBossBeaten && Game.PlayerStats.ChallengeBlobBeaten == false && Game.PlayerStats.ChallengeBlobUnlocked)
                {
                    openSpecialBossDoor = true;
                }
                ...
                break;
            case GameTypes.LevelType.GARDEN:
                if (Game.PlayerStats.FairyBossBeaten && Game.PlayerStats.ChallengeSkullBeaten == false && Game.PlayerStats.ChallengeSkullUnlocked)
                {
                    openSpecialBossDoor = true;
                }
                ...
                break;
            case GameTypes.LevelType.TOWER:
                if (Game.PlayerStats.FireballBossBeaten && Game.PlayerStats.ChallengeFireballBeaten == false && Game.PlayerStats.ChallengeFireballUnlocked)
                {
                    openSpecialBossDoor = true;
                }
            ...
            foreach (var roomObj in RoomList)
            {
                if (roomObj != CurrentRoom)
                {
                    if (roomObj.Bounds.Contains((int) Player.X, (int) Player.Y))
                    {
                        ...
                        foreach (var enemy in CurrentRoom.EnemyList) // Saves all enemy positions in the new room to an array for when the player exits the room.
                        {
                            m_enemyStartPositions.Add(enemy.Position);
                        }

                        foreach (var enemy in CurrentRoom.TempEnemyList) // Saves all enemy positions in the new room to an array for when the player exits the room.
                        {
                        ...
                        foreach (var obj in m_currentRoom.GameObjList)
                        {
    ...
    private void UpdateLevelSong()
    {
        ...
        else if (!(m_currentRoom is StartingRoomObj) && SoundManager.IsMusicPlaying)
        {
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (IsPaused == false)
        {
            ...
            if (DisableRoomTransitioning == false && CollisionMath.Intersects(new Rectangle((int) Player.X, (int) Player.Y, 1, 1), Camera.Bounds) == false)
            {
                CheckForRoomTransition();
            }

            if ((m_inputMap.Pressed(INPUT_LEFTCONTROL) == false || (m_inputMap.Pressed(INPUT_LEFTCONTROL) && (LevelEV.RunDemoVersion || LevelEV.CreateRetailVersion))) && CameraLockedToPlayer)
            {
            ...
            {
                if (m_compassDisplayed && CurrentRoom.Name != "Compass")
                {
                    HideCompass();
                }
            }
            ...
            if (m_objectivePlate.X == 1170)
            {
                ...
                var objectivePlateAbsRect = m_objectivePlate.Bounds;
    ...
    public override void HandleInput()
    {
        ...
        if (Player != null && (m_inputMap.Pressed(INPUT_LEFTCONTROL) == false || (m_inputMap.Pressed(INPUT_LEFTCONTROL) && (LevelEV.RunDemoVersion || LevelEV.CreateRetailVersion))) && Player.IsKilled == false)
        {
            Player.HandleInput();
        }
    ...
    private void UpdateCompass()
    {
        if (m_compassDoor == null && CurrentRoom.Name != "Ending" && CurrentRoom.Name != "Boss" && CurrentRoom.Name != "Start" && CurrentRoom.Name != "Tutorial" && CurrentRoom.Name != " ChallengeBoss")
        {
            ...
            foreach (var room in RoomList)
            {
                ...
                if (room.Name != "Ending" && room.Name != "Tutorial" && room.Name != "Boss" && room.Name != "Secret" && room.Name != "Bonus" && hasEnemies && room.Name != "ChallengeBoss")
                {
            ...
            if (acceptableRooms.Count > 0)
            {
                ...
                foreach (var terrainObj in roomToLink.TerrainObjList)
                {
                    ...
                    {
                        if (terrainObj.Y - door.Y < closestGround && CollisionMath.Intersects(terrainObj.Bounds, new Rectangle((int) door.X, (int) (door.Y + (terrainObj.Y - door.Y) + 5), door.Width, door.Height / 2)))
                        {
        ...
        {
            m_compass.Rotation = CDGMath.AngleBetweenPts(Player.Position, new Vector2(m_compassDoor.Bounds.Center.X, m_compassDoor.Bounds.Center.Y));
        }
    }
    ...
    public void InitializeRenderTargets()
    {
        ...
        RenderTarget = new RenderTarget2D(Camera.GraphicsDevice, GlobalEV.SCREEN_WIDTH, GlobalEV.SCREEN_HEIGHT, false, SurfaceFormat.Color, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
    ...
    public override void Draw(GameTime gameTime)
    {
        ...
        if (CurrentRoom != null && Camera.Zoom == 1 && (m_inputMap.Pressed(INPUT_LEFTCONTROL) == false || (m_inputMap.Pressed(INPUT_LEFTCONTROL) && (LevelEV.RunDemoVersion || LevelEV.CreateRetailVersion))))
        {
        ...
        if (LevelEV.ShowEnemyRadii)
        {
            foreach (var enemy in m_currentRoom.EnemyList)
            {
    ...
    public void SpawnDementiaEnemy()
    {
        ...
        if (enemyObjList.Count > 0)
        {
            ...
            var newEnemy = EnemyBuilder.BuildEnemy(enemyList[CDGMath.RandomInt(0, enemyList.Length - 1)], null, null, null, GameTypes.EnemyDifficulty.BASIC, true);
    ...
    public void ResetEnemyPositions()
    {
        ...
        for (var i = 0; i < m_tempEnemyStartPositions.Count; i++)
        {
    ...
    public void RunGameOver()
    {
        ...
        // A personal caused death! Wow!
        if (m_objKilledPlayer is not DeathLinkObj)
        {
            var name = (ScreenManager.Game as Game)!.ArchipelagoManager.Alias;
            var cause = $"{name}'s ";

            if (m_objKilledPlayer != null)
            {
                if (m_objKilledPlayer is EnemyObj enemy)
                {
                    if (enemy.Difficulty == GameTypes.EnemyDifficulty.MINIBOSS || enemy is EnemyObj_LastBoss)
                    {
                        cause += string.Format(LocaleBuilder.getResourceString("LOC_ID_GAME_OVER_SCREEN_3_NEW"), Game.NameHelper(), LocaleBuilder.getResourceString(enemy.LocStringID));
                    }
                    else
                    {
                        cause += string.Format(LocaleBuilder.getResourceString("LOC_ID_GAME_OVER_SCREEN_4_NEW"), Game.NameHelper(), LocaleBuilder.getResourceString(enemy.LocStringID));
                    }
                }
                else if (m_objKilledPlayer is ProjectileObj projectile)
                {
                    ...
                    {
                        cause += string.Format(LocaleBuilder.getResourceString("LOC_ID_GAME_OVER_SCREEN_5_NEW"), Game.NameHelper());
                    }
                }
                else if (m_objKilledPlayer is HazardObj)
                {
                    cause += string.Format(LocaleBuilder.getResourceString("LOC_ID_GAME_OVER_SCREEN_6_NEW"), Game.NameHelper());
                }
            }
            else
            {
                cause += string.Format(LocaleBuilder.getResourceString("LOC_ID_GAME_OVER_SCREEN_7_NEW"), Game.NameHelper());
            }

            (ScreenManager.Game as Game)!.ArchipelagoManager.SendDeathLink(cause);
        }
    ...
    public void DamageAllEnemies(int damage)
    {
        ...
        var tempEnemyList = new List<EnemyObj>(); // Necessary because TempEnemyList is a list that is continually modified.
    ...
    public override void OnEnter()
    {
        ...
        Player.DisableAllWeight = false; // Fixes bug where you translocate right before enter the castle, resulting in screwed up gravity.
