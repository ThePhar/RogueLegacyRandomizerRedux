...
public class LineageScreen : Screen
{
    ...
    private          int              m_debugNameArrayIndex;
    ...
    public override void LoadContent()
    {
        ...
        var playerTitle = new TextObj(Game.JunicodeFont);
        ...
        var className = playerTitle.Clone() as TextObj;
        ...
        var classDescription = new KeyIconTextObj(Game.JunicodeFont);
        ...
        for (var i = 0; i < 2; i++)
        {
            var traitName = className.Clone() as TextObj;
            ...
            var traitDescription = className.Clone() as TextObj;
        ...
        var spellName = className.Clone() as TextObj;
        ...
        var spellDescription = new KeyIconTextObj(Game.JunicodeFont);
        ...
        var startingPos = Vector2.Zero; // The starting position for the current branch to be added.
    ...
    {
        var selectedObj = m_currentBranchArray[m_selectedLineageIndex];

        var knightName = m_descriptionPlate.GetChildAt(1) as TextObj;
        try
        {
        ...
        var className = m_descriptionPlate.GetChildAt(2) as TextObj;
        ...
        var classDescription = m_descriptionPlate.GetChildAt(3) as KeyIconTextObj;
        ...
        var trait1Name = m_descriptionPlate.GetChildAt(4) as TextObj;
        trait1Name.Y = classDescription.Y + classDescription.Height + 5;
        var trait1Description = m_descriptionPlate.GetChildAt(5) as TextObj;
        trait1Description.Y = trait1Name.Y + 30;

        var spellY = (int) trait1Name.Y;
        ...
        var trait2Name = m_descriptionPlate.GetChildAt(6) as TextObj;
        trait2Name.Y = trait1Description.Y + trait1Description.Height + 5;
        var trait2Description = m_descriptionPlate.GetChildAt(7) as TextObj;
        ...
        if (Game.PlayerStats.HasProsopagnosia)
        {
        ...
        var spellName = m_descriptionPlate.GetChildAt(8) as TextObj;
        ...
        spellName.Y = spellY;
        var spellDescription = m_descriptionPlate.GetChildAt(9) as KeyIconTextObj;
    ...
    ///     Set this to false if you don't want to create random traits for this lineage. Useful when
    ///     only loading lineages.
    /// </param>
    /// <param name="randomizePortrait">
    ///     Whether to randomize the player portrait or not. Setting false sets all armor pieces to
    ///     1.
    /// </param>
    /// <param name="setActive">
    ///     Whether this is an active branch or not. Active branches will be coloured, non-actives will be
    ///     greyed out.
    /// </param>
    private void AddLineageRow(int numLineages, Vector2 position, bool createEmpty, bool randomizePortrait)
    {
        ...
        int[] yPosArray3 = { -450, 0, 450 };
        int[] yPosArray2 = { -200, 200 };

        for (var i = 0; i < numLineages; i++)
        {
            var obj = new LineageObj(this, createEmpty);
    ...
    public void LoadFamilyTreeData()
    {
        ...
        var currentEra = GameEV.BASE_ERA;
        ...
        if (Game.PlayerStats.FamilyTreeArray != null && Game.PlayerStats.FamilyTreeArray.Count > 0)
        {
            ...
            foreach (var treeData in Game.PlayerStats.FamilyTreeArray)
            {
    ...
    public void LoadCurrentBranches()
    {
        if (Game.PlayerStats.CurrentBranches == null || Game.PlayerStats.CurrentBranches.Count < 1)
        {
            ...
            var branchArray = new List<PlayerLineageData>();
            for (var i = 0; i < m_currentBranchArray.Count; i++)
            {
                var newBranch = new PlayerLineageData();
        ...
        else
        {
            ...
            var loadedLineageArray = Game.PlayerStats.CurrentBranches;
            for (var i = 0; i < m_currentBranchArray.Count; i++)
            {
    ...
    {
        var delayAmount = 1 / 60f;
        var volume = m_storedMusicVol;
        var volumeAmount = m_storedMusicVol / 120f;
        for (var i = 0; i < 120; i++)
        {
            Tween.RunFunction(delayAmount * i, this, "ReduceMusic", volume);
            volume -= volumeAmount;
        }

        Tween.RunFunction(2, this, "StopLegacySong");
    ...
    {
        if (m_background != null && m_background.isContentLost)
        {
            ReinitializeRTs();
        }
    ...
    public override void HandleInput()
    {
        if (m_lockControls == false)
        {
            ...
            {
                var previousLineageObj = m_selectedLineageObj;
                var previousLineageIndex = m_selectedLineageIndex;
    ...
    private void HandleDebugInput()
    {
        ...
        var lineageObjChanged = false;

        // if (InputManager.JustPressed(Keys.OemQuotes, null) || InputManager.JustPressed(Keys.OemSemicolon, null))
        // {
        //     (ScreenManager.Game as Game).InitializeFemaleNameArray(true);
        //     (ScreenManager.Game as Game).InitializeMaleNameArray(true);
        // }

        var nameArray = Game.NameArray;
        if (m_currentBranchArray[m_selectedLineageIndex].IsFemale)
        {
            nameArray = Game.FemaleNameArray;
        }

        var previousNameIndex = m_debugNameArrayIndex;
        ...
        var currentClass = m_currentBranchArray[m_selectedLineageIndex].Class;
    ...
    public override void Draw(GameTime gameTime)
    {
        ...
        foreach (var lineageObj in m_masterArray)
        {
            lineageObj.Draw(Camera);
        }

        foreach (var lineageObj in m_currentBranchArray)
        {
            lineageObj.Draw(Camera);
        }
    ...
    public override void Dispose()
    {
        if (IsDisposed == false)
        {
            ...
            foreach (var obj in m_currentBranchArray)
            {
                obj.Dispose();
            }
            ...
            foreach (var obj in m_masterArray)
            {
    ...
    {
        var nameCopies = 0;
        foreach (var data in m_masterArray)
        {
    ...
    {
        foreach (var data in m_currentBranchArray)
        {
